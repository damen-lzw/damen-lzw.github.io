...
module.exports = function (id, value, text, description, initialIndex, section,sectionVal) {
  this.id = id;
  this.value = value;
  this.text = text;
...
Tree.prototype.generateSelections = function () {
  ...
  this.$originalSelect.find('> option').each(function () {
    ...
    var sectionVal = option.getAttribute('data-section-val');
    ...
    var optionObj = new Option(id, optionValue, optionName, optionDescription, optionIndex, section,sectionVal);
    if (optionIndex) {
...
function createSelected(option, disableRemoval, showSectionOnSelected) {
  ...
  if (showSectionOnSelected) {
    var sectionSpan = createNode('span', { class: 'section-name', text: option.section,sectionVal:option.sectionVal });
